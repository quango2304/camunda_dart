...
class TaskVariableApi {
  TaskVariableApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> deleteTaskVariableWithHttpInfo(String id, String varName,) async {
    // ignore: prefer_const_declarations
    final path = r'/task/{id}/variables/{varName}'
      .replaceAll('{id}', id)
  ...
  /// * [String] varName (required):
  ///   The name of the variable to be removed.
  Future<void> deleteTaskVariable(String id, String varName,) async {
    final response = await deleteTaskVariableWithHttpInfo(id, varName,);
  ...
  Future<Response> getTaskVariableWithHttpInfo(String id, String varName, { bool? deserializeValue, }) async {
    // ignore: prefer_const_declarations
    final path = r'/task/{id}/variables/{varName}'
      .replaceAll('{id}', id)
      .replaceAll('{varName}', varName);
  ...
  Future<VariableValueDto?> getTaskVariable(String id, String varName, { bool? deserializeValue, }) async {
    final response = await getTaskVariableWithHttpInfo(id, varName,  deserializeValue: deserializeValue, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  ...
  Future<Response> getTaskVariableBinaryWithHttpInfo(String id, String varName,) async {
    // ignore: prefer_const_declarations
    final path = r'/task/{id}/variables/{varName}/data'
      .replaceAll('{id}', id)
      .replaceAll('{varName}', varName);
  ...
  Future<MultipartFile?> getTaskVariableBinary(String id, String varName,) async {
    final response = await getTaskVariableBinaryWithHttpInfo(id, varName,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///   Determines whether serializable variable values (typically variables that store custom Java objects) should be deserialized on the server side (default `true`). If set to `true`, a serializable variable will be deserialized on server side and transformed to JSON using [Jackson's](https://github.com/FasterXML/jackson) POJO/bean property introspection feature. Note that this requires the Java classes of the variable value to be on the REST API's classpath.  If set to `false`, a serializable variable will be returned in its serialized format. For example, a variable that is serialized as XML will be returned as a JSON string containing XML.  Note: While `true` is the default value for reasons of backward compatibility, we recommend setting this parameter to `false` when developing web applications that are independent of the Java process applications deployed to the engine.
  Future<Response> getTaskVariablesWithHttpInfo(String id, { bool? deserializeValues, }) async {
    // ignore: prefer_const_declarations
    final path = r'/task/{id}/variables'
      .replaceAll('{id}', id);
  ...
  Future<Map<String, VariableValueDto>?> getTaskVariables(String id, { bool? deserializeValues, }) async {
    final response = await getTaskVariablesWithHttpInfo(id,  deserializeValues: deserializeValues, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> modifyTaskVariablesWithHttpInfo(String id, { PatchVariablesDto? patchVariablesDto, }) async {
    // ignore: prefer_const_declarations
    final path = r'/task/{id}/variables'
      .replaceAll('{id}', id);
  ...
  ///
  /// * [PatchVariablesDto] patchVariablesDto:
  Future<void> modifyTaskVariables(String id, { PatchVariablesDto? patchVariablesDto, }) async {
    final response = await modifyTaskVariablesWithHttpInfo(id,  patchVariablesDto: patchVariablesDto, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
  ...
  Future<Response> putTaskVariableWithHttpInfo(String id, String varName, { VariableValueDto? variableValueDto, }) async {
    // ignore: prefer_const_declarations
    final path = r'/task/{id}/variables/{varName}'
      .replaceAll('{id}', id)
      .replaceAll('{varName}', varName);
  ...
  ///
  /// * [VariableValueDto] variableValueDto:
  Future<void> putTaskVariable(String id, String varName, { VariableValueDto? variableValueDto, }) async {
    final response = await putTaskVariableWithHttpInfo(id, varName,  variableValueDto: variableValueDto, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
  ...
  Future<Response> setBinaryTaskVariableWithHttpInfo(String id, String varName, { MultipartFile? data, String? valueType, }) async {
    // ignore: prefer_const_declarations
    final path = r'/task/{id}/variables/{varName}/data'
      .replaceAll('{id}', id)
      .replaceAll('{varName}', varName);
  ...
  /// * [String] valueType:
  ///   The name of the variable type. Either Bytes for a byte array variable or File for a file variable.
  Future<void> setBinaryTaskVariable(String id, String varName, { MultipartFile? data, String? valueType, }) async {
    final response = await setBinaryTaskVariableWithHttpInfo(id, varName,  data: data, valueType: valueType, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
