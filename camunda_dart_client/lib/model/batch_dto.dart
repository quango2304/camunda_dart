...
class BatchDto {
  /// Returns a new [BatchDto] instance.
  BatchDto({
    this.id,
    this.type,
    this.totalJobs,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is BatchDto &&
    other.id == id &&
    other.type == type &&
  ...
  /// Returns a new [BatchDto] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static BatchDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "BatchDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "BatchDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<BatchDto> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <BatchDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BatchDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BatchDto> mapFromJson(dynamic json) {
    final map = <String, BatchDto>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BatchDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of BatchDto-objects as value to a dart map
  static Map<String, List<BatchDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<BatchDto>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = BatchDto.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
