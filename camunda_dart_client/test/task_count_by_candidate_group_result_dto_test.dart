//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for TaskCountByCandidateGroupResultDto
void main() {
  // final instance = TaskCountByCandidateGroupResultDto();

  group('test TaskCountByCandidateGroupResultDto', () {
    // The name of the candidate group. If there are tasks without a group name, the value will be `null`
    // String groupName
    test('to test the property `groupName`', () async {
      // TODO
    });

    // The number of tasks which have the group name as candidate group.
    // int taskCount
    test('to test the property `taskCount`', () async {
      // TODO
    });


  });

}
